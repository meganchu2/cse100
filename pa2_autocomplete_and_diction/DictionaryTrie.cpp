...
bool DictionaryTrie::insert(std::string word, unsigned int freq)
{
  ...
  Node* curr = root;
  bool newWord = false; // will be true if we make a new path ( = new word)
  while(itr != word.end())
  {
    int ind = index(*itr);
    if(*itr == ' ')
    ...
    {
      newWord = true; // once we create new path for letter, we have new word
      curr->letters[ind] = new Node(); // create new path to new node
      ++itr;
      curr = curr->letters[ind];
    }
  } // we have traversed every letter in word
  if(newWord) // if we made new path to insert new word
  {
    curr->frequency = freq;
    curr->word = true;
    return true;
  }
  else if(curr->word) // not new word but duplicate word
  {
  ...
  else // new word without making new path
  {
    curr->frequency = freq;
    curr->word = true;
    return true;
